...
if LongPress(200) {  ; Check if Space key is held down for more than 200ms
	...
	if WinActive("ahk_class Chrome_WidgetWin_1 ahk_exe Code.exe") {
		...
		Send, {Enter} ; Action when VS Code is not active
	}

} else {
	Gosub, SpacebarLabel  ; Action for short press
}
return
...
#If VIM_NORMAL_SPACE_MODE
	...
	Else If (g = "0") {
		...
		Send, v
		gosub, Vim_VisualLabel ;jump to the first non-blank character of the line
	}
	Return
...
/*
		...
		INSERT_MODE_II := false
*/
		Gosub, Gui1Setup  ; Call the setup for the specific GUI Gui1Setup
	} else {
		Send, i
		;guiOpen := false
		CHECK_IS_ON_VIM_NORMAL_SPACE_MODE := false
